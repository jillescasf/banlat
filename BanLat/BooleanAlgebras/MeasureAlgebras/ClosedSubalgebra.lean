...
obtaining by the restriction of a measure algebra to a `σ`-complete Boolean subalgebra.

Moreover, it shows that the following conditions are equivalent:
* `A` is closed under countable suprema, that is, is `σ`-complete.
* `A` is a closed subset of `B` (with respect to the metric `d(a, b) = μ(a ∆ b)`.)
...
adjoining elements, and closures of unions. It also shows the posibility of extending
measure-preserving order isomorphisms to closures.
-/
...
/-- A Boolean subalgebra is closed in a measure algebra if its carrier is
...
/-- The density character of a Boolean subalgebra for the metric induced by
...
/-- The density character of the `σ`-complete closure of `A` is at most the
cardinality `#A` of `A`. -/
theorem subalgebraDensityCharacter_closure_le_mk
    (μ : MeasureAlgebra B) (A : BooleanSubalgebra B) :
    μ.subalgebraDensityCharacter (μ.closure A) ≤ Cardinal.mk A := by
  ...
  have hA_dense : Dense (A' : Set (μ.closure A)) := by
    ...
    intro b r hr
    have hb := b.property
    change (b : B) ∈ _root_.closure (A : Set B) at hb
    rw [Metric.mem_closure_iff] at hb
    obtain ⟨a, ha, hab⟩ := hb r hr
    refine ⟨⟨a, μ.le_closure A ha⟩, ?_, ha⟩
    rw [Metric.mem_ball]
    change dist a (b : B) < r
    simpa only [dist_comm] using hab
  calc
    TopologicalSpace.densityCharacter (μ.closure A) ≤ Cardinal.mk A' :=
      TopologicalSpace.densityCharacter_le_mk_of_dense hA_dense
    _ = Cardinal.mk A := Cardinal.mk_congr (μ.orderIsoSubalgebraInClosure A).toEquiv.symm
...
/- The conditional representation of `A.adjoin b` bounds its density character
by `densityCharacter(A) * densityCharacter(A)`. -/
...
/-- If `κ ≥ ℵ₀` and `densityCharacter(A) < κ`, then adjoining any `b ∈ B`
still gives `densityCharacter(A.adjoin b) < κ`. -/
...
/-- If `ℵ₀ ≤ κ` and `densityCharacter(A) ≤ κ`, then adjoining `b ∈ B`
still gives `densityCharacter(A.adjoin b) ≤ κ`. -/
...
`A i` has density character at most `κ`, then the
`σ`-complete closure of `⋃ i, A i` also has density character at most `κ`. -/
...
theorem exists_measureAlgebraEquiv_closure_extends
